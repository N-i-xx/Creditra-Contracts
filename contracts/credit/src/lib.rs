...
use types::{CreditLineData, CreditStatus, ProtocolConfig, RateChangeConfig};
...
mod test {
    ...
    fn test_repay_credit_integration_uses_mocked_allowance_and_balance_state() {
        ...
        // repay is state-only; token balances/allowances are unchanged
    ...
    #[test]
    fn test_get_protocol_config_defaults_after_init() {
        let env = Env::default();
        env.mock_all_auths();
        let admin = Address::generate(&env);
        let contract_id = env.register(Credit, ());
        ...
        assert!(cfg.liquidity_token.is_none());
        assert_eq!(cfg.liquidity_source, Some(contract_id));
        assert!(cfg.rate_change_config.is_none());
    }

    #[test]
    fn test_get_protocol_config_with_liquidity_token_set() {
        let env = Env::default();
        env.mock_all_auths();
        let admin = Address::generate(&env);
        let contract_id = env.register(Credit, ());
        ...
        let cfg = client.get_protocol_config();
        assert_eq!(cfg.liquidity_token, Some(token_address));
    }

    #[test]
    fn test_get_protocol_config_with_liquidity_source_set() {
        let env = Env::default();
        env.mock_all_auths();
        let admin = Address::generate(&env);
        let reserve = Address::generate(&env);
        ...
        let cfg = client.get_protocol_config();
        assert_eq!(cfg.liquidity_source, Some(reserve));
    }

    #[test]
    fn test_get_protocol_config_with_rate_change_config_set() {
        let env = Env::default();
        env.mock_all_auths();
        let admin = Address::generate(&env);
        let contract_id = env.register(Credit, ());
        ...
        let cfg = client.get_protocol_config();
        let rate_cfg = cfg.rate_change_config.expect("rate_change_config should be set");
        assert_eq!(rate_cfg.max_rate_change_bps, 500);
        assert_eq!(rate_cfg.rate_change_min_interval, 3600);
    }

    #[test]
    fn test_get_protocol_config_all_fields_set() {
        let env = Env::default();
        env.mock_all_auths();
        let admin = Address::generate(&env);
        let reserve = Address::generate(&env);
        ...
        assert_eq!(cfg.liquidity_token, Some(token_address));
        assert_eq!(cfg.liquidity_source, Some(reserve));
        let rate_cfg = cfg.rate_change_config.unwrap();
        assert_eq!(rate_cfg.max_rate_change_bps, 200);
        assert_eq!(rate_cfg.rate_change_min_interval, 7200);
    }
}
